<script>
	...
	const items = [
		...
		{ label: 'WRITING', href: '/writing', match: (p) => p.startsWith('/writing') },
...
<header class="site-header">
	<a class="wordmark" href="/">
		...
		<span class="name">Springfield Yonga</span>
	</a>
	...
	<nav class="nav-desktop" aria-label="Primary">
		{#each items as item (item.label)}
			...
			{#if item.href}
				...
				>
			{:else}
				<span class="nav-item nav-item--stub" title="Coming soon">{item.label}</span>
			{/if}
		{/each}
	</nav>
...
{#if menuOpen}
	<nav class="nav-mobile" aria-label="Primary">
		{#each items as item (item.label)}
			{@const active = item.match(path)}
			{#if item.href}
				<a class="nav-item" class:active href={item.href} onclick={() => (menuOpen = false)}
					>{item.label}</a
				>
			{:else}
